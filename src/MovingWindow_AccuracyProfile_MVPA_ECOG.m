function [TPR,FPR,NT,ND] = MovingWindow_AccuracyProfile_MVPA_ECOG(condition, subjects, holdouts, varargin)
...
% DEPENDENCIES
% 1. glmnet
% 2. sqrt_truncate_r.m (from WholeBrain_MVPA/src)
    p = inputParser();
    if isdeployed
        ...
        addParameter(p, 'metadata', 'cvpartition_10fold_wholewindow.mat');
        ...
        addParameter(p, 'metadata', 'C:/Users/mbmhscc4/MATLAB/src/MovingWindow_MVPA_ECOG/data/cvpartition_10fold_wholewindow.mat');
        parse(p, condition, subjects, holdouts, varargin{:});
        fprintf('condition: %s\n', p.Results.condition);
        fprintf('subjects: %d\n', p.Results.subjects);
        fprintf('holdouts: %d\n', p.Results.holdouts);
    ...
    for i = 1:N
        ...
        for wj = 1:nwindows
            ...
            Yz_test = glmnetPredict(fitObj(wi), X_test);
        ...
        if w == LASTWINDOW;
            ...
            save(resultfile, 'TPR', 'FPR', 'NT', 'ND','windows');
            ps = s;
            ph = h;
        end
    end
