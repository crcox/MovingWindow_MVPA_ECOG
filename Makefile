...
all: setup glmnet MovingWindow_MVPA_ECOG MovingWindow_AccuracyProfile_MVPA_ECOG clean-postbuild
...
MovingWindow_MVPA_ECOG: $(SRC)/MovingWindow_MVPA_ECOG.m
	$(MCC) -v $(MFLAGS) $(INCL) -a $(DATA) -o $@ $<

MovingWindow_AccuracyProfile_MVPA_ECOG: $(SRC)/MovingWindow_AccuracyProfile_MVPA_ECOG.m
	$(MCC) -v $(MFLAGS) $(INCL) -a $(DATA) -o $@ $<
...
clean-all:
	-rm MovingWindow_MVPA_ECOG
	-rm MovingWindow_AccuracyProfile_MVPA_ECOG
	-rm $(DEP)
	-rm $(DATA)
	-rm $(SRC)
